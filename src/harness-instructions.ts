...
export const defaultHarnessToolSpecs: HarnessToolSpec[] = [
  {
    ...
    description: [
      ...
      '',
      'Every preparation returns a preparationLineageId identifying the logical request it belongs to.',
      '',
      ...
      '- "revise_request": the request is incomplete. Inspect validationIssues and hints to determine what is missing. If the task provides enough information, fix the request and call prepare_paid_request again, passing the same preparationLineageId so the revision supersedes the incomplete preparation. Otherwise stop and explain what is still missing.',
      ...
      'Do not reuse a preparationLineageId across unrelated requests; only pass it when revising the same logical request.',
